...
library V3TwapLib {
    ...
    /// @notice Returns the arithmetic mean tick over a historical observation window.
    /// @dev Negative non-integral means round toward negative infinity to match Uniswap V3 oracle semantics.
    /// @param pool Pool whose observation history is queried.
    /// @param period Number of seconds in the lookback window.
    /// @return avgTick Time-weighted average tick over the requested window.
    function getTwapTick(IUniswapV3Pool pool, uint32 period) internal view returns (int24 avgTick) {
        ...
        int56 periodInt56 = int56(uint56(period));
        avgTick = int24(tickDiff / periodInt56);

        if (tickDiff < 0 && tickDiff % periodInt56 != 0) {
            avgTick--;
        }
    }
    ...
    function getTwapPrice(IUniswapV3Pool pool, uint32 period) internal view returns (uint256 price) {
        int24 avgTick = getTwapTick(pool, period);
