...
1. Reject if `shares` is zero.
2. Read `totalSupply()` before burning.
3. Read the current `token0` and `token1` balances held by the vault.
4. Compute the proportional token amounts owed to the user.
5. Burn the user's shares.
6. Transfer `token0` and `token1` to the user.
