...
This wires volatility-based V3 range selection into the strategy layer while keeping slippage minimum calculation as a separate future controller.
...
- the strategy uses adapter-reported deployed amounts, not an independent market quote
- execution still withdraws all tracked venue liquidity before redeploying the target plan
- V3 target params generated by the strategy still use zero minimum amounts until the dynamic slippage controller is added
